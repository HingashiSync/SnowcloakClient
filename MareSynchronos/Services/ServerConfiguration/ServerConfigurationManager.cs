...
public class ServerConfigurationManager
{
    ...
    private void EnsureMainExists()
    {
        ...
        for (int i = 0; i < _configService.Current.ServerStorage.Count; ++i)
        {
            ...
            if (x.ServerUri.Equals(ApiController.HingashiServiceUri, StringComparison.OrdinalIgnoreCase))
                elfExists = true;
        }
        if (!elfExists)
        {
            _logger.LogDebug("Re-adding missing server {uri}", ApiController.HingashiServiceUri);
            _configService.Current.ServerStorage.Insert(0, new ServerStorage() { ServerUri = ApiController.HingashiServiceUri, ServerName = ApiController.HingashiServer });
            if (_configService.Current.CurrentServer >= 0)
                _configService.Current.CurrentServer++;
        }
        Save();
