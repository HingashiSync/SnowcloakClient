...
public class MarePlugin : MediatorSubscriberBase, IHostedService
{
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        Logger.LogInformation("Launching {name} {major}.{minor}.{build}.{rev}", "Hingashi Sync", version.Major, version.Minor, version.Build, version.Revision);
        Mediator.Publish(new EventMessage(new Services.Events.Event(nameof(MarePlugin), Services.Events.EventSeverity.Informational,
            $"Starting Hingashi Sync {version.Major}.{version.Minor}.{version.Build}.{version.Revision}")));
        ...
        try
        {
            ...
            if (_mareConfigService.Current.LogLevel != LogLevel.Information)
            {
                Mediator.Publish(new NotificationMessage("Abnormal Log Level",
                    $"Your log level is set to '{_mareConfigService.Current.LogLevel}' which is not recommended for normal usage. Set it to '{LogLevel.Information}' in \"Hingashi Settings -> Debug\" unless instructed otherwise.",
